...
export class Storage implements IStorage {
  getQuestions(domain?: string, difficulty?: string, limit?: number): Question[] {
    ...
    if (difficulty) { conditions.push("difficulty = ?"); args.push(difficulty); }
  ...
  completeSession(id: number, correctCount: number): Session | undefined {
    const r = sqlite.prepare(
      "UPDATE sessions SET correct_count = ?, completed_at = ? WHERE id = ? RETURNING *"
  ...
  getSessionAnswers(sessionId: number): Answer[] {
    const rows = sqlite.prepare("SELECT * FROM answers WHERE session_id = ?").all(sessionId) as any[];
    return rows.map(mapAnswer);
  }

  getQuestionStats(questionId: number): QuestionStat | undefined {
    const r = sqlite.prepare("SELECT * FROM question_stats WHERE question_id = ?").get(questionId) as any;
  ...
  getOverallStats(): { totalAttempted: number; totalCorrect: number; totalSessions: number; avgScore: number } {
    ...
    const sessionsResult = sqlite.prepare(
